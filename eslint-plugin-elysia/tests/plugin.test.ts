...
describe("plugin shape", () => {
  it("exports a plugin object with name and version meta", () => {
    expect(plugin.meta?.name).toBe("eslint-plugin-elysia");
  ...
  it("attaches a recommended config under the `elysia` namespace", () => {
    const recommended = plugin.configs?.recommended;
    expect(recommended).toBeDefined();
    expect(recommended?.plugins?.elysia).toBe(plugin);
  });
});
...
describe("recommended config", () => {
  ...
  it("recommended-config rule keys all use the `elysia/` prefix", () => {
    for (const key of Object.keys(recommendedRules)) {
      expect(key.startsWith("elysia/")).toBe(true);
    }
  });
});
