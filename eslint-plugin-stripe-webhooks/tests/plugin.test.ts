...
describe("plugin shape", () => {
  it("exports a plugin object with name and version meta", () => {
    expect(plugin.meta?.name).toBe("eslint-plugin-stripe-webhooks");
  ...
  it("attaches a recommended config under the `stripe-webhooks` namespace", () => {
    const recommended = plugin.configs?.recommended;
    expect(recommended).toBeDefined();
    expect(recommended?.plugins?.["stripe-webhooks"]).toBe(plugin);
  });
});
...
describe("recommended config", () => {
  ...
  it("recommended-config rule keys all use the `stripe-webhooks/` prefix", () => {
    for (const key of Object.keys(recommendedRules)) {
      expect(key.startsWith("stripe-webhooks/")).toBe(true);
    }
  });
});
